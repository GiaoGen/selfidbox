...
## 9. Task Completion — Update PROJECT_STATUS.md

After every completed task, update `TASKER_STATUS.md` in the repo root:

* Record what was done, what changed, and what the current state is.
* If the task introduced new files, new components, or new capabilities, list them.
* If the task changed architecture or data flow, note it.
* Keep it concise — a few bullet points per entry, with the most recent entry at the top.

Do NOT skip this step. Do NOT claim the task is done until `TASKER_STATUS.md` has been updated.
