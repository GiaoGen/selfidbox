...
export function TrendingCard({ site, rank }: { site: TestSite; rank: number }) {
    ...
    >
      ...
      <div className="relative mt-6 flex items-center justify-between">
        <div className="flex items-center gap-3 text-xs text-white/70">
          <span>{site.estimatedMinutes} min</span>
          {site.popularity_score != null && (
            <span className="rounded-full bg-white/15 px-2 py-0.5 text-white/85 backdrop-blur-sm">
              🔥 {Math.round(site.popularity_score)}
            </span>
          )}
        </div>
